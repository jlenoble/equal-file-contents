...
describe("Testing equalFileContents", () => {
  const cwd = process.cwd();

  describe(`Pipe is noop()`, () => {
      ...
      tmpDir("tmp", () => {
        return streamToPromise(
          gulp
            ...
            .pipe(babel())
            .pipe(gulp.dest("tmp"))
        ).then(() => equalFileContents("gulp/**/*.js", "tmp", babel));
      })
    );
  ...
  describe(`Dest is outside the package directory tree`, () => {
    beforeEach(function() {
      this.dest = "/tmp/equalFileContents-test_" + new Date().getTime();
    });

    it(`equalFileContents returns a promise that resolves on equality`, function() {
      return streamToPromise(
        gulp.src("gulp/**/*.js", { base: cwd }).pipe(gulp.dest(this.dest))
      ).then(() => equalFileContents("gulp/**/*.js", this.dest));
    });

    it(`equalFileContents returns a promise that rejects on inequality`, function() {
      return streamToPromise(
        gulp.src("gulp/**/*.js", { base: cwd }).pipe(gulp.dest(this.dest))
      )
        .then(() => equalFileContents("gulp/**/*.js", this.dest, babel))
        .catch(err => {
          expect(err.message).to.match(/expected .* to equal/);
        });
    });
  ...
  describe(`Dest is deeper inside the package directory tree`, () => {
      ...
      tmpDir(base, function() {
        return streamToPromise(
          ...
          .then(() =>
            streamToPromise(
              gulp
                .src(path.join(this.src, "**/*.js"), { base })
                .pipe(gulp.dest(this.dest))
            )
          )
          .then(() =>
            equalFileContents(
              path.join(this.src, "**/*.js"),
              this.dest,
              undefined,
              base
            )
  ...
  describe("Testing with more than 16 files", () => {
    it(
      `equalFileContents returns a promise that resolves on equality`,
      tmpDir("tmp", () => {
        mkdirp.sync("tmp");
